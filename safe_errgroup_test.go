...
func TestZeroGroup(t *testing.T) {
	...
	for _, tc := range cases {
		...
		for i, err := range tc.errs {
			err := err
			g.SafeGo(context.Background(), func() error { return err })

			if firstErr == nil && err != nil {
				firstErr = err
			}

			if gErr := g.SafeWait(); gErr != firstErr {
				t.Errorf("after %T.SafeGo(context.Background(), func() error { return err }) for err in %v\n"+
					"g.SafeWait() = %v; want %v",
...
func TestTryGo(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		if !g.SafeTryGo(context.Background(), fn) {
...
func BenchmarkGo(b *testing.B) {
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		g.SafeGo(context.Background(), func() error { fn(); return nil })
	}
	g.SafeWait()
}
